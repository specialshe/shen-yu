...
END_OF_JAVA
	...
	for (my $r = 1; $r <= $max_row{$index}; $r++) {
		...
		# ページ内リンク 
		    ...
		    unless (defined $option->{no_overlib}) {
			$cellheader .= qq(onMouseOver="return overlib('$inline{$coordinate{$index}{$r}{$c}{option}{LABEL}}', ol_offsetx=-20, ol_textsize=2, ol_width=600, ol_sticky=1, ol_cap=' ', ol_closesize=4, ol_close='×')" );
